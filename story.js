...
function funonload() {
	var mediaQuery = window.matchMedia('(max-device-width: 812px) and (orientation: portrait)');
	...
	if (PHONE) {
		leftnav = document.getElementsByClassName('story_video_mobnavigation_left')[0];
		rightnav = document.getElementsByClassName('story_video_mobnavigation_right')[0];
	} else {
		leftnav = document.getElementsByClassName('story_button_left')[0];
		rightnav = document.getElementsByClassName('story_button_right')[0];
	}
	
	var playnav = document.getElementsByClassName('story_video_mobnavigation_play')[0];
	var playnav_container = document.getElementsByClassName('story_video_mobnavigation_play_outer')[0];
	
	playnav.addEventListener('click', function(){firstplay(videoEl, playnav_container, playBtnSVG)}, false);
	
	leftnav.addEventListener('click', function(){prevstory(videoEl);}, false);
	rightnav.addEventListener('click', function(){nextstory(videoEl);}, false);
	videoEl.addEventListener('ended', function(){nextstory(videoEl);}, false);
	...
	videoEl.addEventListener('timeupdate', function(){bartime(videoEl);}, false);
	...
	videoEl.addEventListener('loadstart', function(){if (PHONE) videoEl.muted = false;}, false);
}
...
function prevstory(videoEl) {
	if (current == 0) {
		...
		current--;
		videoEl.pause();
		$('#storysource').attr("src", "uploads/" + videolinks[current]);
		$('div.story_header_bar_part_current').remove();
		$('#story'+current).children('.story_header_bar_part_white').remove();
		$('#story'+current).append('<div class="story_header_bar_part_white story_header_bar_part_current" style="width: 0%;"></div>');
		videoEl.load();
		videoEl.play();
	}
}

function nextstory(videoEl) {
	if (current == numstories-1) {
		window.location = "index.php";
	} else {
		current++;
		videoEl.pause();
		$('#storysource').attr("src", "uploads/" + videolinks[current]);
		$('.story_header_bar_part_current').width("100%");
		$('.story_header_bar_part_current').attr("class", "story_header_bar_part_white");
		$('#story'+current).append('<div class="story_header_bar_part_white story_header_bar_part_current" style="width: 0%;"></div>');
		videoEl.load();
		videoEl.play();
	}
...
function firstplay(videoEl, playnav_container, playBtnSVG) {
	videoEl.play();
	playBtnSVG.innerHTML = svg_pause;
	playnav_container.style.display = "none";
	videoEl.muted = false;
}

function bartime(videoEl) {
	var wdth = videoEl.currentTime / videoEl.duration;
	$('.story_header_bar_part_current').width((wdth * 100) + "%");
}
