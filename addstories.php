			...
			if (window.FormData === undefined) {
				...
				$.ajax({
					...
					success: function(msg){
						if (msg.error == '') {
							$("#js-file").hide();
							$('#result').html('<img src="uploads/'+msg.url+'" style="max-height: 350px;"/><br/>Кадр из истории (Превью)');
							$('#video').attr("value", msg.url_original);
							$('#video').attr("readonly", "readonly");
							$('#image').attr("value", msg.url);
...
?>
		<?php
		$result = mysqli_query($link, "SELECT * FROM `stories`;") or die (mysqli_error($link));
		?>
		...
		<tr>
			...
			<td><?php
			$epin = $row['pin'];
			$pinresult = mysqli_query($link, "SELECT * FROM `pins` WHERE `id`='$epin';") or die (mysqli_error($link));
			$pinrow = mysqli_fetch_array($pinresult, MYSQLI_ASSOC);
			echo $pinrow['name']; ?>
			</td>
			<td><a href="uploads/<?php echo $row['video']; ?>">link...</a></td>
			<td><img src="uploads/<?php echo $row['image']; ?>" style="max-height: 300px;"><br/>Кадр из видео (превью)</td>
