...
<!DOCTYPE html>
	...
	<head>
		<meta charset="utf-8">
		<title>Все исполняемые страницы</title>
...
?>
		<?php
		$result = mysqli_query($link, "SELECT * FROM `pages`;") or die (mysqli_error($link));
		?>
		...
		<table class="whitebox whitebox-table whitebox-table-colors whitebox-table-first whitebox-table-secondcenter">
			<tr>
				...
				<td>Название</td>
				<td>Адрес</td>
				<td><abbr title="Уровень доступа отображается в том случае, если для него не указана роль">Минимальная роль (уровень доступа)</abbr></td>
...
<tr>
			...
			<td><a href="<?php echo $row['address']; ?>"><?php echo $row['address']; ?></a></td>
			<td><?php
